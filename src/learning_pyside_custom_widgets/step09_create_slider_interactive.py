...
class QCircularSlider(QtWidgets.QWidget):
    ...
    DEFAULTS: SliderDeaults = {
        ...
        "indicatorColor": QtGui.QColor("#FFF"),
        "intensity": 30,
    }
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        self._createIndicator(painter, circularRadius, angle)
    ...
    def _createIndicator(self, painter: QtGui.QPainter, length: int, angle: float):
        radius = int(length / 2)
        sinTheta = math.sin(math.radians(angle))
        cosTheta = math.cos(math.radians(angle))

        indicatorPos = Point(
            x=int(-radius * sinTheta) + radius,
            y=int(radius * cosTheta) + radius
        )

        # outside
        pen = QtGui.QPen(self.indicatorColor, 2)
        painter.setBrush(QtCore.Qt.BrushStyle.NoBrush)
        painter.setPen(pen)
        painter.drawEllipse(indicatorPos.x + 1, indicatorPos.y + 1,
                            self.indicatorSize - 2, self.indicatorSize - 2)

        # inside
        painter.setBrush(QtGui.QBrush(self.indicatorColor))
        painter.drawEllipse(indicatorPos.x + 5, indicatorPos.y + 5,
                            self.indicatorSize - 10, self.indicatorSize - 10)

    def mouseReleaseEvent(self, event: QtGui.QMouseEvent):
        self.mousePosX = event.globalPosition().x()
        self.mousePosY = event.globalPosition().y()


    def mouseMoveEvent(self, event: QtGui.QMouseEvent):
        self.addValue = int((event.globalPosition().x() - self.mousePosX) / self.intensity)
        self.setValue(max(0, min(100, self.value + self.addValue)))
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent):
        self.mousePosX = event.globalPosition().x()
        self._path.moveTo(event.position().toPoint())
        self.update()
...
if __name__ == "__main__":
    ...
    circularSlider.valueChanged.connect(spinbox.setValue)
